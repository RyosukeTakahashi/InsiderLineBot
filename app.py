...
from constants import line_bot_api, db, client, parser, port
    ...
    for event in events:
        ...
        if isinstance(event, MessageEvent):
            ...
            if isinstance(event.message, TextMessage):
                ...
                if text in ['f', 'ふ']:
                    ...
                    line_bot_api.reply_message(
                        event.reply_token,
                        TextSendMessage(text="changing answer state false")
                    )
        ...
        if isinstance(event, PostbackEvent):
            ...
            if next == 'answered':
                ...
                single_turn_guess_insider(room, room_id, rooms_dict, start_timestamp)
...
def sample_timer(event):
    ...
    q.enqueue(set_reminders, timestamp, [3, 6, 9], members)
...
def single_turn_guess_insider(room, room_id, rooms_dict, start_timestamp):


    time_left = int(time.time()) - start_timestamp
    line_bot_api.multicast(
        room['members'],
        [TextSendMessage(text=f'正解が出たようです。それではインサイダーは誰だったか、議論しましょう。時間は{time_left}秒で答えが出たので、{time_left}秒です。')]
    )
...
def single_turn_main(room, room_id, event):
    ...
    reminder_timings = [0, 3, 6]
    q = Queue(connection=r)
    q.enqueue(set_reminders, start_timestamp, reminder_timings, members)
...
# for testing
def add_start_time_to_json(event):

    with open('rooms.json', 'r') as room_json:
        rooms_dict = json.load(room_json)
        rooms_dict["1"]["rounds_info"][-1]["start_timestamp"] = int(event.timestamp)
    with open('rooms.json', 'w') as room_json:
        json.dump(rooms_dict, room_json, indent=2)
